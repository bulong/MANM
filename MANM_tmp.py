...
def create_model(embedding_size, encoder_size,max_step,batch_size):
    ...
    print("***************")
    print(K.int_shape(p_sentence_input))
    ...
    """
    # Concat Attention，Wc的每一步没有共享参数

    Wc1 = K.random_normal_variable(shape=(2 * encoder_size, max_step), mean=0, scale=1)
    Wc2 = K.random_normal_variable(shape=(2 * encoder_size, max_step), mean=0, scale=1)
    vc = K.random_normal_variable(shape=(max_step, 1), mean=0, scale=1)

    whq = K.dot(q_sentence_output,Wc1) #每个banch内矩阵乘法，shape:(batch_size,max_step,max_step)
    whp = K.dot(p_sentence_output,Wc2) #shape:(batch_size,max_step,max_step)

    sc = tf.multiply(tf.tanh(tf.add(whp,tf.transpose(whq,perm=[0, 2, 1]))),vc) #(batch_size,max_step,max_step)
    ac = tf.nn.softmax(sc) #按列softmax，(batch_size,max_step,max_step)
    qc = K.batch_dot(ac, q_sentence_output) #shape:(batch_size,max_step,2 * encoder_size)


    # Bilinear Attention
    Wb = K.random_normal_variable(shape=(2 * encoder_size, 2 * encoder_size), mean=0, scale=1)
    #sb= tf.matmul(tf.matmul(p_sentence_output,Wb),tf.transpose(q_sentence_output))#(max_step,max_step)
    sb = K.batch_dot(K.dot(p_sentence_output, Wb), tf.transpose(q_sentence_output, [0, 2, 1])) #(batch_size,max_step,max_step)
    ab = tf.nn.softmax(sb)  # 按列softmax，(max_step,max_step)
    qb = K.batch_dot(ab, q_sentence_output)  # shape:(batch_size,max_step,2 * encoder_size)

    # Dot Attention,Wd共享参数
    #Wd = K.random_normal_variable(shape=(2 * encoder_size, max_step), mean=0, scale=1)
    Wd = K.random_normal_variable(shape=(2 * encoder_size, 1), mean=0, scale=1)
    vd = K.random_normal_variable(shape=(max_step, 1), mean=0, scale=1)

    q_p_dot = tf.expand_dims(q_sentence_output,axis = 1)*tf.expand_dims(p_sentence_output,axis = 2)

    sd = tf.multiply(tf.tanh(K.dot(q_p_dot, Wd)), vd)#(batch_size,max_step,max_step)
    sd = tf.squeeze(sd,axis = -1)
    ad = tf.nn.softmax(sd)#按列softmax，(batch_size,max_step,max_step)
    qd = K.batch_dot(ad, q_sentence_output)  # shape:(batch_size,max_step,2 * encoder_size)

    # Minus Attention,Wm共享参数
    Wm = K.random_normal_variable(shape=(2 * encoder_size, 1), mean=0, scale=1)
    vm = K.random_normal_variable(shape=(max_step, 1), mean=0, scale=1)
    #q_p_minus = q_sentence_output - p_sentence_output
    q_p_minus = tf.expand_dims(q_sentence_output, axis=1) - tf.expand_dims(p_sentence_output, axis=2)

    sm = tf.multiply(tf.tanh(K.dot(q_p_minus, Wm)), vm)
    sm = tf.squeeze(sm, axis=-1)
    am = tf.nn.softmax(sm)  # 按列softmax，(batch_size,max_step,max_step)
    qm = K.batch_dot(am, q_sentence_output)  # shape:(batch_size,max_step,2 * encoder_size)
    """
    ...
    """
    Wg = K.random_normal_variable(shape=(4 * encoder_size, 1), mean=0, scale=1)
    xc = tf.concat([qc, p_sentence_output], axis=2)
    xb = tf.concat([qb, p_sentence_output], axis=2)
    xd = tf.concat([qd, p_sentence_output], axis=2)
    xm = tf.concat([qm, p_sentence_output], axis=2)
    #四个门，shape:(batch_size,max_step,1)
    gc = tf.sigmoid(K.dot(xc, Wg))
    gb = tf.sigmoid(K.dot(xb, Wg))
    gd = tf.sigmoid(K.dot(xd, Wg))
    gm = tf.sigmoid(K.dot(xm, Wg))
    # shape:(batch_size,max_step,2*encoder_size)
    xc_ = gc * xc
    xb_ = gb * xb
    xd_ = gd * xd
    ...
    """
    #mixed aggregation
    h_cat = K.reshape(K.concatenate([hc, hb, hd, hm], axis=2), (batch_size, max_step, 4, 2 * encoder_size))#shape:(batch_size,max_step,4,2 * encoder_size),每一step有4个h
    W1 = K.random_normal_variable(shape=(2 * encoder_size, 1), mean=0, scale=1)
    W2 = K.random_normal_variable(shape=(max_step, 1), mean=0, scale=1)
    va = K.random_normal_variable(shape=(1, 4), mean=0, scale=1)
    v = K.random_normal_variable(shape=(max_step, 1), mean=0, scale=1)

    w1_h = K.dot(h_cat, W1)  # 每个banch内矩阵乘法，shape:(batch_size,max_step,4,1)
    #w1_h_ = K.reshape(w1_h, (batch_size, max_step, 4))
    w1_h_ = tf.squeeze(w1_h, axis=-1)

    #w2_va = K.dot(W2, va)  # shape:(batch_size,max_step,max_step)
    s = tf.multiply(tf.add(w1_h_,K.dot(W2,va)), v)
    a = tf.nn.softmax(s)
    x = K.batch_dot(a, h_cat)
    mix_agg_encoder = Bidirectional(GRU(encoder_size, return_sequences=True), merge_mode='concat')
    h = mix_agg_encoder(x)
    ...
    #Prediction Layer
    Wq1 = K.random_normal_variable(shape=(2 * encoder_size, 1), mean=0, scale=1)
    Wq2 = K.random_normal_variable(shape=(max_step, 1), mean=0, scale=1)
    vq = K.random_normal_variable(shape=(1, 1), mean=0, scale=1)
    sq = tf.multiply(tf.add(K.dot(q_sentence_output, Wq1), K.dot(Wq2, vq)), v)
    sq = tf.squeeze(sq, axis=-1)
    aq = tf.nn.softmax(sq)  # 按列softmax，(batch_size,max_step)
    rq = K.batch_dot(aq, q_sentence_output)  # 求得q_sentence_output所有步长的加权平均(句子q的向量表示),shape:(batch_size,2 * encoder_size)

    Wp1 = K.random_normal_variable(shape=(2 * encoder_size, 1), mean=0, scale=1)
    Wp2 = K.random_normal_variable(shape=(2 * encoder_size, max_step), mean=0, scale=1)
    m1_ = K.dot(h, Wp1)  # 每个banch内矩阵乘法，shape:(batch_size,max_step,1)

    m2_ = tf.expand_dims(K.dot(rq, Wp2),axis=-1)  # shape:(batch_size,max_step,1)
    sr = tf.multiply(tf.tanh(tf.add(m1_,m2_)), v)  # (batch_size,max_step,max_step)
    sr = tf.squeeze(sr, axis=-1)
    ar = tf.nn.softmax(sr)  # 按列softmax，(batch_size,max_step,max_step)
    rp = K.batch_dot(ar, h)  # shape:(batch_size,2 * encoder_size),句子p的向量表示
    prediction = Dense(1, activation='sigmoid', input_shape=(None, 2 * encoder_size))
